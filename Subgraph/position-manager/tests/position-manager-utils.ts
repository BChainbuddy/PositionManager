...
export function createDexRouterRemovedEvent(
  dexRouter: Address
): DexRouterRemoved {
  let dexRouterRemovedEvent = changetype<DexRouterRemoved>(newMockEvent())

  dexRouterRemovedEvent.parameters = new Array()

  dexRouterRemovedEvent.parameters.push(
    new ethereum.EventParam("dexRouter", ethereum.Value.fromAddress(dexRouter))
  )

  return dexRouterRemovedEvent
}
